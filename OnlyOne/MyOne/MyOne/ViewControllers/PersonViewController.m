...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.row == 0) {
		cell.textLabel.text = @"默认用户";
		cell.imageView.image = [UIImage imageNamed:@"tabbar_item_reading_selected"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.row == 0) {// 点击进入个人中心
        //再次创建 新出来的 二合一 一体化
        //参数一 title 参数二 消息 参数三 选择效果（actionSheet 、alert）
        UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"注意" message:@"个人中心尚未完善,请使用默认账户进行阅读" preferredStyle:UIAlertControllerStyleAlert];
